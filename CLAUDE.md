...
### 6. OS/브라우저 환경 차이는 코드 레벨로만 점검
OS별(Windows/macOS) 실제 렌더링 차이, 폰트 표시, 스크롤바 
스타일 등은 코드로 검증할 수 없다. CSS/폰트 관련 코드에서 
OS별 분기 처리가 있는지까지만 확인하고, "실제로 두 OS에서 
동일하게 보임"이라고 단정하지 않는다. 이런 항목은 
"실기기/실OS 확인 필요"로 별도 표시한다.
...
### 기능 구현 전
- 새 기능 만들기 전, 기존 구조 확장 가능성과 충돌 여부를 먼저 검토하고 계획을 보고한 뒤 승인받고 코드 작성
- "이거 나중에 편집 가능하게 바꿀 가능성 있나?"를 항상 먼저 물어보기

### 날짜/시간 처리
- 날짜 관련 로직은 반드시 `parseDateStr()` + KST 기준으로 통일
- `new Date(string)` 직접 사용 금지 — Safari/RN 환경별 파싱 차이 발생
- 시간 비교는 항상 `getTime()` 기반으로 (float 오차 방지)

### 입력 유효성 검사
- 사용자 입력을 검증 없이 그대로 저장하는 필드 만들지 말기
  (예: 종료일 < 시작일, 빈 문자열, 음수 등은 반드시 guard)

### 리스트/배열 로직
- 정렬, 필터, 그룹핑 짤 때 "겹치지 않는 경우"와 "겹치는 경우" 모두 테스트 케이스로 짚어보기
- 빈 배열, 1개, 100개일 때도 동작하는지 확인

### UI 정합성
- UI 텍스트(버튼 이름, 레이블)와 실제 동작이 일치하는지 항상 확인
- 새 기능은 사용자 눈에 잘 띄는 곳에 노출 (숨겨진 더블클릭/우클릭 기능 지양)

### 3중 검증 루틴 (기능 완성 후 반드시 수행)
1. `npx tsc --noEmit` — 타입 오류 0개 확인
2. 테스트 실행 (`npm test`) — 기존 테스트 통과 확인
3. 실기기/실브라우저 직접 확인 (크롬 + 사파리, iOS + Android)

### 앱(React Native) 개발 추가 규칙
- 기능 하나씩 완성 후 실기기 테스트 전까지 다음 기능으로 넘어가지 말기
- SafeAreaView, KeyboardAvoidingView 반드시 적용 (노치/홈바 대응)
- 플랫폼별 분기는 `Platform.OS` 사용, 하드코딩 금지
- Firestore onSnapshot 구독은 컴포넌트/훅 unmount 시 반드시 unsubscribe
